...
// trySendingBatch tries to send a batch if necessary. If sending needs to be
// retried it will return true.
func (q *queue) trySendingBatch(done <-chan struct{}) bool {
	q.batchMtx.Lock()
	defer q.batchMtx.Unlock()
	if len(q.batch) == 0 {
		return false
